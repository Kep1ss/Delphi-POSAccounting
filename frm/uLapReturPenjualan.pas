...
type
  TFlapReturPenjualan = class(TForm)
...
var
  FlapReturPenjualan: TFlapReturPenjualan;
  stk1,stk2 : string;
  str : string;
...
procedure TFlapReturPenjualan.btn1Click(Sender: TObject);
...
begin
  ...
  with Dlaporan.QreturJual do begin
    ...
    if (Cpelanggan.Checked) and (str <> '') then
       SQL.Add(' AND namapelanggan in ('+str+') ');

    Open;
  end;


  Dlaporan.Fr1.LoadFromFile(aPathRpt+'\frReturPenjualan.fr3');

  Dlaporan.Fr1.Preview := frxPreview1;
  Dlaporan.Fr1.ShowReport();
end;

procedure TFlapReturPenjualan.btn2Click(Sender: TObject);
begin
  Dlaporan.Fr1.PrepareReport(True) ;
  Dlaporan.fr1.Export(Dlaporan.frxls1);
end;

procedure TFlapReturPenjualan.btnClick(Sender: TObject);
begin
  frxPreview1.Print;
end;

procedure TFlapReturPenjualan.C1Click(Sender: TObject);
var
  i : integer;
  x : string;
begin
...
procedure TFlapReturPenjualan.CpelangganClick(Sender: TObject);
begin
  ls.Enabled := Cpelanggan.Checked;
end;

procedure TFlapReturPenjualan.CperiodeClick(Sender: TObject);
begin
  dt1.Enabled := Cperiode.Checked;
  dt2.Enabled := Dt1.Enabled;
end;

procedure TFlapReturPenjualan.FormShow(Sender: TObject);
begin
  dt1.Date := now;
  dt2.Date := now;
  with dm.Q1 do begin
...
procedure TFlapReturPenjualan.lsClickCheck(Sender: TObject);
var
  i : integer;
  x : string;
begin
